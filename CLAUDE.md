...
| Independent reviewer | **code-reviewer** Claude subagent (Opus) | After every Codex diff — different vendor = genuinely independent second opinion |
...
| Code review after Codex (or me) writes a change | `code-reviewer` subagent — every time, not optional |
...
4. **Review.** `code-reviewer` subagent on the diff. Not optional.
